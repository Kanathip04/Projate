...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'restore_booking') {
        ...
        if ($id > 0) {
            ...
            header("Location: {$currentPage}?msg=".urlencode("กู้คืนข้อมูลเรียบร้อยแล้ว")."&type=success"); exit;
        }
    }
...
.bk-banner-archive { background: linear-gradient(135deg, #374151 0%, #4b5563 100%); }
.bk-stat { border-top-color: var(--muted); }
</style>

<div class="bk-wrap">

  <div class="bk-banner bk-banner-archive">
    <div class="bk-banner-text">
      <h1>รายการย้อนหลัง</h1>
      <p>ข้อมูลที่ถูกจัดเก็บแล้ว — สามารถกู้คืนหรือลบถาวรได้</p>
    </div>
                ...
                <td>
                  <div class="bk-actions">
                    <form method="POST" class="bk-inline" onsubmit="return confirm('กู้คืนรายการนี้?')">
                      ...
                      <button class="bk-btn bk-btn-success" style="font-size:0.75rem;padding:7px 12px;">↺ กู้คืน</button>
                    </form>
                    <form method="POST" class="bk-inline" onsubmit="return confirm('ลบถาวร — ไม่สามารถกู้คืนได้!')">
                      <input type="hidden" name="action" value="delete_booking">
                      <input type="hidden" name="id" value="<?= (int)['id'] ?>">
                      <button class="bk-btn bk-btn-danger" style="font-size:0.75rem;padding:7px 12px;">🗑 ลบถาวร</button>
                    </form>
                  </div>
                </td>
              </tr>
          ...
          <?php else: ?>
            <tr><td colspan="9">
              <div class="bk-empty">
                ...
                <div class="bk-empty-text">ยังไม่มีข้อมูลที่จัดเก็บ<br><?=  ? 'ลองเปลี่ยนคำค้นหา' : 'รายการที่จัดเก็บจะปรากฏที่นี่' ?></div>
              </div>
            </td></tr>
          <?php endif; ?>
        </tbody>
