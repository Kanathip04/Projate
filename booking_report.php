...
$roomCheckinToday  = (int)$conn->query("SELECT COALESCE(SUM(guests),0) n FROM room_bookings WHERE DATE(created_at)='$today' AND booking_status='approved' AND archived=0")->fetch_assoc()['n'];
$roomCheckinMonth  = (int)$conn->query("SELECT COALESCE(SUM(guests),0) n FROM room_bookings WHERE DATE(created_at) BETWEEN '" . date('Y-m-01') . "' AND '" . date('Y-m-t') . "' AND booking_status='approved' AND archived=0")->fetch_assoc()['n'];
$roomCheckinYear   = (int)$conn->query("SELECT COALESCE(SUM(guests),0) n FROM room_bookings WHERE DATE(created_at) BETWEEN '{$thisYear}-01-01' AND '{$thisYear}-12-31' AND booking_status='approved' AND archived=0")->fetch_assoc()['n'];
$roomCheckinPeriod = (int)$conn->query("SELECT COALESCE(SUM(guests),0) n FROM room_bookings WHERE " . dateWhere('created_at',$dateFrom,$dateTo) . " AND booking_status='approved' AND archived=0")->fetch_assoc()['n'];
