...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'restore_booking') {
        ...
        if ($id > 0) {
            ...
            $message = "กู้คืนข้อมูลเรียบร้อยแล้ว"; $message_type = "success";
        }
    }
    if ($action === 'delete_booking') {
        ...
        if ($id > 0) {
            ...
            $st->bind_param("i", $id); $st->execute(); $st->close();
            $message = "ลบข้อมูลถาวรเรียบร้อยแล้ว"; $message_type = "success";
        }
    }
...
$rs = $conn->query("SELECT COUNT(*) t FROM room_bookings WHERE archived=1");
$stat_archive = (int)$rs->fetch_assoc()['t'];
...
$pageTitle = "รายการย้อนหลัง"; $activeMenu = "booking_archive";
...
.bk-banner{border-radius:18px;padding:26px 32px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;position:relative;overflow:hidden;background:linear-gradient(135deg,#374151 0%,#4b5563 100%);}
.bk-banner::before{content:'';position:absolute;width:280px;height:280px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,0.08) 0%,transparent 70%);top:-90px;right:-60px;pointer-events:none;}
.bk-banner h1{font-family:'Playfair Display',serif;font-style:italic;font-size:1.5rem;color:#fff;margin:0 0 5px;}
.bk-banner p{font-size:0.8rem;color:rgba(255,255,255,0.72);margin:0;}
...
.bk-stat{background:var(--card);border-radius:14px;padding:20px;box-shadow:0 2px 12px rgba(26,26,46,.06);border-top:3px solid var(--muted);}
...
.bk-btn-success{background:#f0fdf4;color:var(--success);border:1.5px solid #86efac;}.bk-btn-success:hover{background:#dcfce7;}
...
.unit-pill{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;font-size:0.65rem;font-weight:700;background:rgba(107,114,128,.1);border:1px solid rgba(107,114,128,.28);color:#374151;}
</style>
...
<div class="bk-wrap">
  ...
  <div class="bk-banner">
    <div>
      ...
      <p>ข้อมูลที่ถูกจัดเก็บแล้ว — กู้คืนหรือลบถาวรได้</p>
    </div>
    <div class="bk-banner-links">
      <a href="admin_booking_list.php"     class="bk-banner-link">← รายการรออนุมัติ</a>
      <a href="admin_booking_approved.php" class="bk-banner-link">✅ รายการอนุมัติ</a>
    </div>
  </div>
  ...
  <div class="bk-stats">
    <div class="bk-stat">
      <div class="bk-stat-label">รายการที่จัดเก็บทั้งหมด</div>
      <div class="bk-stat-value"><?= $stat_archive ?></div>
      <div class="bk-stat-sub">ข้อมูลที่ archived = 1</div>
    </div>
  </div>
  ...
  <div class="bk-card">
    <div class="bk-card-header">
      <div>
        <div class="bk-card-title">รายการข้อมูลที่จัดเก็บแล้ว</div>
        <div class="bk-card-sub">กู้คืนหรือลบถาวรได้จากที่นี่</div>
      </div>
              ...
              <tr>
                ...
                <td>
                  <div class="bk-actions">
                    <form method="POST" class="bk-inline" onsubmit="return confirm('กู้คืนรายการนี้?')">
                      <input type="hidden" name="action" value="restore_booking">
                      <input type="hidden" name="id" value="<?= (int)$row['id'] ?>">
                      <button class="bk-btn bk-btn-success" style="padding:6px 11px;font-size:0.74rem;">↺ กู้คืน</button>
                    </form>
                    <form method="POST" class="bk-inline" onsubmit="return confirm('ลบถาวร — ไม่สามารถกู้คืนได้!')">
                      <input type="hidden" name="action" value="delete_booking">
                      <input type="hidden" name="id" value="<?= (int)$row['id'] ?>">
                      <button class="bk-btn bk-btn-danger" style="padding:6px 11px;font-size:0.74rem;">🗑 ลบถาวร</button>
                    </form>
                  </div>
                </td>
              </tr>
            ...
            <tr><td colspan="8">
              <div class="bk-empty">
                ...
                <div class="bk-empty-text">
                  <?= $search ? 'ไม่พบรายการที่ตรงกับ "'.h($search).'"' : 'ยังไม่มีข้อมูลที่จัดเก็บ' ?>
                </div>
              </div>
            </td></tr>
          <?php endif; ?>
