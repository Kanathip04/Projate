...
$whereParts = ["visit_date = CURDATE()"];
if ($search !== '') {
    $safe = $conn->real_escape_string($search);
    $whereParts[] = "(nickname LIKE '%$safe%' OR user_type LIKE '%$safe%')";
}
$where = "WHERE " . implode(" AND ", $whereParts);

/*
 |-------------------------------
 | ตรวจสอบว่ามีคอลัมน์ visit_time หรือไม่
 |-------------------------------
*/
$has_visit_time = false;
$colCheck = $conn->query("SHOW COLUMNS FROM tourists LIKE 'visit_time'");
if ($colCheck && $colCheck->num_rows > 0) {
    $has_visit_time = true;
}

$selectFields = "id, nickname, user_type, visit_date, created_at";
if ($has_visit_time) {
    $selectFields .= ", visit_time";
}

$sql = "SELECT $selectFields FROM tourists $where ORDER BY id DESC";
$result = $conn->query($sql);
if (!$result) {
    die("SQL Error (dashboard list): " . $conn->error);
}

$count_sql = "SELECT user_type, COUNT(*) AS total
              FROM tourists
              WHERE visit_date = CURDATE()
              GROUP BY user_type";
$count_result = $conn->query($count_sql);
if (!$count_result) {
    die("SQL Error (dashboard count): " . $conn->error);
}
...
.lm-table tbody td:nth-child(1) {
  width: 50px;
  text-align: center;
  color: var(--muted);
}
.lm-table thead th:nth-child(4),
.lm-table tbody td:nth-child(4) {
  width: 100px;
  text-align: center;
}
.lm-table thead th:nth-child(5),
.lm-table tbody td:nth-child(5) {
  width: 100px;
  text-align: center;
}
...
<div class="dash-grid">
  ...
  foreach ($type_counts as $type => $cnt):
      ...
      $i++;
  ?>
    <div class="stat-card" style="border-top-color:<?= h($c) ?>">
      <div class="stat-label"><?= h($type) ?></div>
      <div class="stat-value" style="color:<?= h($c) ?>"><?= (int)$cnt ?></div>
      <div class="stat-unit">คน</div>
    </div>
  <?php endforeach; ?>
</div>
      ...
      <?php if (!empty($type_counts)): ?>
        <div class="type-list">
          ...
          foreach ($type_counts as $type => $cnt):
              ...
              $bc  = $barColors[$j % count($barColors)];
              $j++;
          ?>
            <div>
              <div class="type-row" style="margin-bottom:4px;">
                <span class="type-label"><?= h($type) ?></span>
                <span style="font-size:.7rem;color:var(--muted)"><?= (int)$pct ?>%</span>
                <span class="type-count"><?= (int)$cnt ?> คน</span>
              </div>
              <div class="type-bar-wrap">
                <div class="type-bar" style="width:<?= (int)$pct ?>%;background:<?= h($bc) ?>"></div>
              </div>
            </div>
          <?php endforeach; ?>
        </div>
      ...
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        <form method="GET" style="display:flex;gap:8px;align-items:center;">
          <div class="search-wrap">
            <input type="text" name="search" placeholder="ค้นหา..." value="<?= h($search) ?>">
          </div>
        ...
        <a href="archive_today.php" class="btn btn-accent btn-sm" onclick="return confirm('ต้องการปิดวันหรือไม่?')">ปิดวัน</a>
      </div>
    </div>
        ...
        <tbody id="liveTable">
          <?php if ($result && $result->num_rows > 0): ?>
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
              ...
              $badgeClass = ($userType === 'นักศึกษา')
                  ? 'badge-blue'
                  : (($userType === 'บุคลากร') ? 'badge-orange' : 'badge-green');

              $t = '-';

              if ($has_visit_time && !empty($row['visit_time'])) {
              ...
              <tr>
                <td><?= $n++ ?></td>
                <td><strong><?= h($row['nickname'] ?? '-') ?></strong></td>
                <td><span class="badge <?= h($badgeClass) ?>"><?= h($userType) ?></span></td>
                <td><?= h($t) ?></td>
                <td>
                  <a class="btn btn-danger btn-sm"
                     href="delete_tourist.php?id=<?= (int)($row['id'] ?? 0) ?>"
          ...
          <?php else: ?>
            <tr>
              <td colspan="5" class="empty-row">ยังไม่มีข้อมูลสำหรับวันนี้</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
