...
// ✅ activeMenu = 'profile' เพื่อให้ sidebar highlight ถูก
...
.ap-hero::before{ content:''; position:absolute; width:320px; height:320px; border-radius:50%; background:radial-gradient(circle,rgba(201,169,110,0.13) 0%,transparent 70%); top:-120px; right:-60px; pointer-events:none; }
.ap-hero::after { content:''; position:absolute; width:180px; height:180px; border-radius:50%; background:radial-gradient(circle,rgba(201,169,110,0.08) 0%,transparent 70%); bottom:-60px; left:-30px; pointer-events:none; }

.ap-av-wrap{ position:relative; flex-shrink:0; z-index:1; }
.ap-av{ width:88px; height:88px; border-radius:50%; background:var(--gold-dim); border:3px solid rgba(201,169,110,0.5); display:flex; align-items:center; justify-content:center; font-family:'Playfair Display',serif; font-style:italic; font-size:2.2rem; color:var(--gold); overflow:hidden; }
.ap-av img{ width:100%; height:100%; object-fit:cover; }
.ap-av-btn{ position:absolute; bottom:2px; right:2px; width:26px; height:26px; border-radius:50%; background:var(--gold); border:2px solid var(--ink); display:flex; align-items:center; justify-content:center; font-size:0.68rem; cursor:pointer; transition:transform .2s; }
.ap-av-btn:hover{ transform:scale(1.1); }

.ap-hero-info{ flex:1; z-index:1; }
.ap-hero-name{ font-family:'Playfair Display',serif; font-style:italic; font-size:1.7rem; color:#fff; margin-bottom:4px; }
.ap-hero-email{ font-size:0.8rem; color:rgba(255,255,255,0.5); margin-bottom:12px; }
.ap-hero-badges{ display:flex; gap:8px; }
.ap-badge{ display:inline-flex; align-items:center; gap:5px; padding:4px 12px; border-radius:20px; font-size:0.68rem; font-weight:700; letter-spacing:.06em; }
...
.ap-input, .ap-textarea{ width:100%; padding:11px 12px 11px 36px; border:1.5px solid var(--border); border-radius:10px; font-family:'Sarabun',sans-serif; font-size:0.88rem; color:var(--ink); background:#fafaf8; outline:none; transition:border-color .2s, box-shadow .2s; }
.ap-input:focus, .ap-textarea:focus{ border-color:var(--gold); background:#fff; box-shadow:0 0 0 3px rgba(201,169,110,.12); }
...
.ap-btn-row{ display:flex; align-items:center; justify-content:flex-end; gap:12px; margin-top:20px; padding-top:18px; border-top:1px solid var(--border); }
.ap-btn{ display:inline-flex; align-items:center; gap:7px; padding:11px 24px; border:none; border-radius:10px; font-family:'Sarabun',sans-serif; font-size:0.85rem; font-weight:700; cursor:pointer; transition:all .2s; letter-spacing:.04em; text-decoration:none; }
...
@media(max-width:760px){ .ap-grid{ grid-template-columns:1fr; } .ap-hero{ flex-direction:column; text-align:center; padding:28px 22px; } }
</style>
        ...
        <div class="ap-card-body">
          <form method="POST" enctype="multipart/form-data" id="profileForm">
            ...
            <div class="ap-fg">
              ...
              <div class="ap-iw">
                ...
                <input class="ap-input" type="text" name="fullname"
                       value="<?= h($user['fullname'] ?? '') ?>" required placeholder="ชื่อ-นามสกุล">
              </div>
            </div>
            ...
            <div class="ap-btn-row">
              <span style="font-size:0.76rem;color:var(--muted);">กดปุ่มเพื่อยืนยันการเปลี่ยนแปลง</span>
            ...
            <div class="ap-fg">
              ...
              <div class="ap-iw">
                ...
                <input class="ap-input" type="password" name="new_password"
                       placeholder="อย่างน้อย 6 ตัวอักษร" required oninput="apCheckStr(this.value)">
              </div>
    ...
    <div>
        ...
        <div class="ap-card-body">
          <div class="ap-info-list">
            <div class="ap-info-item">
              ...
              <div><div class="ap-info-lbl">สถานะ</div><div class="ap-info-val">⚡ Administrator</div></div>
            </div>
            <div class="ap-info-item">
              <div class="ap-info-dot"></div>
              <div><div class="ap-info-lbl">อีเมล</div><div class="ap-info-val" style="word-break:break-all;"><?= h($user['email']) ?></div></div>
            </div>
            <div class="ap-info-item">
              <div class="ap-info-dot"></div>
              <div><div class="ap-info-lbl">วันที่สมัคร</div><div class="ap-info-val"><?= h(date('d M Y', strtotime($user['created_at']))) ?></div></div>
            </div>
            <div class="ap-info-item">
              <div class="ap-info-dot"></div>
              <div><div class="ap-info-lbl">ยืนยันอีเมล</div><div class="ap-info-val"><?= $user['is_verified'] ? '✅ ยืนยันแล้ว' : '⏳ ยังไม่ยืนยัน' ?></div></div>
            </div>
            <div class="ap-info-item">
              <div class="ap-info-dot"></div>
              <div><div class="ap-info-lbl">ใช้งานมาแล้ว</div><div class="ap-info-val"><?= $join_days ?> วัน</div></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="ap-card">
        ...
        <div class="ap-card-body" style="text-align:center;">
          ...
          <label for="ap-avatar-file" class="ap-btn ap-btn-ghost" style="cursor:pointer;width:100%;justify-content:center;display:flex;">
            📷 เลือกรูปภาพ
          </label>
