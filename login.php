    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // plain text (ไม่แนะนำ แต่รองรับไว้ก่อน)
                $passwordMatch = true;
            }
            ...
            if ($passwordMatch) {
                ...
                // จดจำ (remember me) — เก็บ cookie 30 วัน
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --muted: #7a7a8c; --border: #e0ddd6; --danger: #c0392b; --radius: 2px;
    }
    body {
    ...
    .wrapper {
      display: flex; width: 860px; max-width: 96vw; min-height: 520px;
    ...
    .alert-success { background: #edf7ed; border: 1px solid #2e7d32; border-radius: var(--radius); padding: 10px 14px; font-size: 0.82rem; color: #2e7d32; margin-bottom: 18px; }
    ...
    .forgot-link { font-size: 0.82rem; color: var(--accent); text-decoration: none; border-bottom: 1px solid transparent; }
    ...
    .divider-text { font-size: 0.72rem; color: var(--muted); }

    .btn-social { width: 100%; padding: 12px; background: transparent; border: 1.5px solid var(--border); border-radius: var(--radius); font-family: 'Sarabun', sans-serif; font-size: 0.85rem; color: var(--ink); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; transition: border-color 0.2s, background 0.2s; }
    .btn-social:hover { border-color: var(--accent); background: #faf7f2; }

    .register-row { text-align: center; margin-top: 24px; font-size: 0.82rem; color: var(--muted); }
    .register-row a { color: var(--ink); font-weight: 600; text-decoration: none; border-bottom: 1px solid var(--ink); transition: color 0.2s; }
